    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <div className="container mx-auto px-6 py-12 relative">
        <div className="text-center mb-12 animate-fade-in">
          ...
          <h1 className="heading-lg mb-4">Compare Schools</h1>
        ...
        <Card className="glass-card max-w-2xl mx-auto mb-12 border-0 animate-slide-up delay-200">
          ...
          <CardContent>
              ...
              {searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 bg-white border rounded-lg shadow-xl z-10 mt-1 border-border">
                  {searchResults.map((college) => (
                    <div
                      key={college.id}
                      className="p-4 hover:bg-muted cursor-pointer border-b border-border last:border-b-0 first:rounded-t-lg last:rounded-b-lg transition-colors duration-200"
            ...
            {selectedColleges.length >= 4 && (
              <p className="text-sm text-muted-foreground mt-3">Maximum of 4 schools can be compared at once</p>
            )}
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-1 gap-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {selectedColleges.map((college, index) => (
                  <Card key={college.id} className="glass-card border-0 hover-lift" style={{ animationDelay: `${0.4 + index * 0.1}s` }}>
                    <CardHeader className="pb-4">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <CardTitle className="text-lg line-clamp-2 text-foreground mb-2">
                            {college['school.name']}
                          </CardTitle>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                      {selectedColleges.map((college) => (
                        <div key={college.id} className="text-center p-4 bg-muted/30 rounded-xl">
                ...
                <Card className="glass-card border-0">
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      <div>
                        <h4 className="font-semibold mb-4 text-foreground text-lg">In-State Tuition</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                          {selectedColleges.map((college) => (
                            <div key={college.id} className="text-center p-4 bg-green-50 rounded-xl border border-green-200">
                      ...
                      <div>
                        <h4 className="font-semibold mb-4 text-foreground text-lg">Out-of-State Tuition</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                          {selectedColleges.map((college) => (
                            <div key={college.id} className="text-center p-4 bg-blue-50 rounded-xl border border-blue-200">
        ...
        {selectedColleges.length === 0 && (
          <div className="text-center py-20 animate-fade-in delay-400">
            ...
            <h3 className="text-2xl font-medium text-foreground mb-3">No schools selected</h3>
            <p className="text-muted-foreground text-lg">Search and add schools above to start comparing them</p>
          </div>
        )}
      </div>
    </div>
